...
import {onChatClick} from './MainPage'
...
export function Chat(props) {
    ...
    if (chats) {
        for (let j= 0; j < child; j+=1) {
            // alert(chats[j].Messages)
            children.push(
                <button className="chat" date={chats[j].Messages[chats[j].Messages.length - 1].Date} value={j} onClick={onChatClick}>
	...
	return (
        <React.Fragment>
            <div className="chatList">
                {children.sort(compare).reverse()}
            </div>   
            <button className="create_btn_container" onClick={()=>{buildChat(); setChild(child+1)}}>
